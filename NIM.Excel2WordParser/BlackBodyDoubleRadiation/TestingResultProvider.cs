...
namespace NIM.CertificationGenerator.BlackBodyDoubleRadiation
{
    public class TestingResultProvider
    {
        ...
        public TestingProcessResultDto GetResult(SpreadsheetDocument document)
        {
            ...
            this.SetTargetValues(result, document);
        ...
        private void SetTargetValues(TestingProcessResultDto result, SpreadsheetDocument document)
        {
            //精密辐射温度计信息
            //双面波长，
            var sheetName = "记录";
            ...
            var address = "";
            var cellValue = "";
            while (startRowIndex < 1000)
            {
                address = "B" + startRowIndex.ToString();
                cellValue = document.GetCellValue(sheetName, address);
                if (flag.Equals(cellValue))
                {
                    foundRowIndex = startRowIndex + 1;
                    break;
                }
            ...
            if (foundRowIndex == 0)
                throw new Exception("记录工作表的B列，找不到 精密辐射温度计信息");

            address = "B" + foundRowIndex.ToString();
            while (true)
            {
                address = "B" + foundRowIndex.ToString();
                cellValue = document.GetCellValue(sheetName, address);
                ...
                {
                    foundRowIndex++;
                    if (foundRowIndex > 1000)
                    {
                        throw new Exception("记录工作表的B列，取不到精密辐射温度计信息下面的值");
                    }
                }
                else
                    break;
            }
            result.T1V = cellValue;
            result.T1Temperatures = new List<double>();
            startRowIndex = foundRowIndex;
            var endRowIndex = startRowIndex;
            while (true)
            {
                endRowIndex++;
                address = "B" + endRowIndex.ToString();
                cellValue = document.GetCellValue(sheetName, address);
                if (!string.IsNullOrEmpty(cellValue))
                    break;
            ...
            for (var i = startRowIndex; i < endRowIndex; i++)
            {
                ...
                {
                    result.T1Temperatures.Add(v);
                }
            }
            foundRowIndex = endRowIndex;
            address = "B" + foundRowIndex.ToString();
            cellValue = document.GetCellValue(sheetName, address);
            result.T2V = cellValue;
            result.T2Temperatures = new List<double>();
            startRowIndex = foundRowIndex;
            ...
            for (var i = startRowIndex; i < endRowIndex; i++)
            {
                ...
                if (double.TryParse(cellValue, out v))
                {
                    result.T2Temperatures.Add(v);
                }
            }

            //now net's find the code.
            flag = "温度计型号";
            foundRowIndex = 0;
            ...
            while (startRowIndex < 1000)
            {
                address = "A" + startRowIndex.ToString();
                cellValue = document.GetCellValue(sheetName, address);
                if (flag.Equals(cellValue))
                {
                    foundRowIndex = startRowIndex + 1;
            ...
            if (foundRowIndex == 0)
                throw new Exception("记录工作表的A列，找不到 温度计型号");
            var t1Value = result.T1V; result.T1V = "";
            var t2Value = result.T2V; result.T2V = "";
            startRowIndex++;
            while (true)
            {
                address = "A" + startRowIndex.ToString();
                var model = document.GetCellValue(sheetName, address);
                if (string.IsNullOrEmpty(model))
                    break;
                address = "B" + startRowIndex.ToString();
                var code = document.GetCellValue(sheetName, address);

                address = "C" + startRowIndex.ToString();
                var pl = document.GetCellValue(sheetName, address);
                if (t1Value.Contains(model) && t1Value.Contains(code))
                    result.T1V = pl;
                if (t2Value.Contains(model) && t2Value.Contains(code))
                    result.T2V = pl;
                startRowIndex++;
            }


            if (string.IsNullOrEmpty(result.T1V))
            {
                throw new Exception($"精密辐射温度计信息{t1Value}找不到对应的波长");
            }
            if (string.IsNullOrEmpty(result.T2V))
            {
                throw new Exception($"精密辐射温度计信息{t2Value}找不到对应的波长");
            }
        }
        ...
        private void SetStatsticsValues(TestingProcessResultDto result, SpreadsheetDocument document)
        {
            ...
            while (true)
            {
                ...
                double v;
                if (!double.TryParse(obj.温控设置, out v))
                    continue;
                ...
                if (cellValue != "#DIV/0!")
                {
                    ...
                    var cValue = BlackBodyHelpers.GetStaticeNumber(document.GetCellValue(sheetName, "C" + startIndex.ToString()));
                    var dValue = BlackBodyHelpers.GetStaticeNumber(document.GetCellValue(sheetName, "D" + startIndex.ToString()));

                    var savedObj = result.TestingStatstics.SingleOrDefault(t => t.温控设置 == obj.温控设置);
                    if (savedObj == null)      //如果还没有加到系统中去，那就一定是第1个                  第一个的肯定是8-14    第二个的是0.9
                    {
                        if (result.T1Temperatures.Exists(t => t == v) && result.T2Temperatures.Exists(t => t == v))     //如果两个都有，那一定是T1的值
                        {
                            obj.T1亮度温度 = cValue;
                            obj.T1U值 = dValue;
                        }

                        else if (result.T1Temperatures.Exists(t => t == v))
                        {
                            obj.T1亮度温度 = cValue;
                            obj.T1U值 = dValue;
                        }
                        else if (result.T2Temperatures.Exists(t => t == v))
                        {
                            obj.T2亮度温度 = cValue;
                            obj.T2U值 = dValue;
                        }
                    ...
                    else  //如果系统中已经存在了
                    {
                        ...
                        if (result.T1Temperatures.Exists(t => t == v) && result.T2Temperatures.Exists(t => t == v))     //如果两个都有，那一定是T1的值
                        {
                            if (savedObj.T1亮度温度 == TestinStatisticsDto.EMPTY)
                            {
                                savedObj.T1亮度温度 = cValue;
                                savedObj.T1U值 = dValue;
                                hasUpdate = true;
                            }
                            if (!hasUpdate && savedObj.T2亮度温度 == TestinStatisticsDto.EMPTY)
                            {
                                savedObj.T2亮度温度 = cValue;
                                savedObj.T2U值 = dValue;
                            }
                        }
                        ...
                        else
                        {
                            ...
                            if (result.T1Temperatures.Exists(t => t == v) && savedObj.T1亮度温度 == TestinStatisticsDto.EMPTY)
                            {
                                savedObj.T1亮度温度 = cValue;
                                savedObj.T1U值 = dValue;
                                hasUpdate = true;
                            }
                            if (!hasUpdate && result.T2Temperatures.Exists(t => t == v) && result.T2Temperatures.Exists(t => t == v))
                            {
                                savedObj.T2亮度温度 = cValue;
                                savedObj.T2U值 = dValue;
                            }
